...
def train_gcn(x, y, train_mask, val_mask, edge_index, hidden=32, embed_dim=32, lr=1e-4,weight_decay=1e-4,
              num_layers = 12, use_residual=False, epochs=150, patience = 30, print_loss_every=5, plot_loss=False, k_neighbors=70):
                ...
                print(f'Training GCN on {device}. Residual Connections: {use_residual}')
                ...
                if plot_loss:
                  ...
                  plt.grid(alpha = 0.4)
                  plt.tight_layout()
                  plt.savefig(f'results/gcn_training_loss_12layers_K{k_neighbors}_residual{use_residual}.png', dpi=200)
                  plt.show()
